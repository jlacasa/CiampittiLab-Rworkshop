...
quadratic <- function(x, a, b, c){
  mu <- (a + b*x + c*x^2)
  epsilon <- rnorm(length(x),0,.1)
  y <- mu + epsilon
  return(y)
}
x <- seq(0,14, length.out = 100)
y <- quadratic(x, 4,.5,-.026)
...
my_lm2 <- lm(y~x+I(x^2), data = my_data)
...
usda_linear3 <- lm(Value ~ State + fYear, data = usda)

usda_linear4 <- lm(Value ~ State * Year, data = usda)

usda_linear5 <- lm(Value ~ State * fYear, data = usda)
```
